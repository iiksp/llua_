...
n2n=function(n) return tonumber(ntos(n)) end
bexp=BigNum.exp
...
stringsub=function(s,st,ed) return string.sub(s,n2n(st),n2n(ed)) end
...
function vm:bne()
	...
	if not cond or cond == newn(0) then
		self.bs:move(offset)
	end
...
op_func[ADD]=genBinOp(OP_ADD,'ADD')
op_func[MINUS]=genBinOp(OP_MINUS,'ADD')
op_func[MUL]=genBinOp(OP_MUL,'ADD')
op_func[DIV]=genBinOp(OP_DIV,'ADD')
...
function encrypt2file(fnIn,fnBc,fnOut)
	s=file2array(fnIn)
...
encrypt2file('hashtest.txt','punkHash.lluac','encrypted.ph')
--encrypt2file('hashtest.txt','punkHash.lluac','encrypted.ph')